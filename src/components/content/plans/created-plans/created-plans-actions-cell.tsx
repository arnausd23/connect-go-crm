...
const CreatedPlansActionsCell = ({ data }: CreatedPlansActionsCellProps) => {
  ...
  const [editPlanData, setEditPlanData] = useState<IEditPlan>({
    ...
    accessType,
    hasHourRestriction,
    restrictionHours: restrictionHours ? (restrictionHours as number[]) : [15, 18],
    disp: data.disp as "Go" | "Connect" | "Ambos",  // Convertir explícitamente el tipo a uno de los valores permitidos
});
