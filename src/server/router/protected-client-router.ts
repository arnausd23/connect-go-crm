...
.query('getPlans', {
  ...
  async resolve({ input, ctx }) {
    const { skip, take, userName, planName, startingDate, endingDate } = input;
  ...
  .query('exportPlans', {
    ...
    async resolve({ input, ctx }) {
      ...
      const plans: Omit<IExportUserPlans, 'fileName'>[] = userPlans.map((userPlan) => {
        const {
          startingDate,
          endingDate,
          user,
  ...
  .mutation('edit', {
    ...
    async resolve({ input, ctx }) {
      const { id, name, ci, phoneNumber, email, editPhoto, labeledFaceDescriptor } = input;
      ...
      if (editPhoto && labeledFaceDescriptor) {
        ...
        const newDescriptors = Array.from(labeledFaceDescriptor).map((value: unknown) => {
        ...
        const userLabeledFaceDescriptor = await ctx.prisma.labeledFaceDescriptor.findFirst({
          where: { ci: user.ci },
        });
        ...
        if (userLabeledFaceDescriptor) {
          ...
          console.log(`Se actualizó el descriptor facial para el usuario con CI: ${ci}`);
        } else {
          ...
          console.log(`Se creó un nuevo descriptor facial para el usuario con CI: ${ci}`);
        }
      }
    },
  })
  ...
  .mutation('create', {
    ...
    async resolve({ input, ctx }) {
      const { ci, name, phoneNumber, email, labeledFaceDescriptorJson } = input;
  ...
  .mutation('assignPlan', {
    ...
    async resolve({ input, ctx }) {
      const { ci, name, startingDate, endingDate, parking, groupClasses } = input;
